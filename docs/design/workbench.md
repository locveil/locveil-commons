...
- Plugin **source lives in its owning repo** — openapi type generators stay repo-side
  ...
  imports a product repo's TS sources.
- **Consumption = built artifact.** Dev-phase mechanism: the workbench takes `file:`
  dependencies on the sibling repos' *built* plugin packages (dist + embedded generated
  types) — the same sibling-path pattern the eval framework already uses. The final
  distribution mechanism (registry publishing, pinning) is **deferred to the
  productization step**, same deferral class as §5.
